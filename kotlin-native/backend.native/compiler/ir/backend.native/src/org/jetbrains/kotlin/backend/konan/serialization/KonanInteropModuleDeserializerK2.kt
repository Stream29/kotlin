...
internal class KonanInteropModuleDeserializerK2(
    ...
    private val symbolsToLinkLater = mutableMapOf<IdSignature, IrSymbol>()
    ...
    override fun contains(idSig: IdSignature): Boolean {
        ...
        val topLevelSig = ((idSig as? IdSignature.AccessorSignature)?.propertySignature ?: idSig)
                .topLevelSignature() as? IdSignature.CommonSignature ?: return false
    ...
    override fun tryDeserializeIrSymbol(idSig: IdSignature, symbolKind: BinarySymbolData.SymbolKind): IrSymbol? {
        ...
        if (symbolKind != BinarySymbolData.SymbolKind.CLASS_SYMBOL) {
            // Unless it's a class, we cannot compute the actual signature for found declarations right away,
            // and so cannot link the declarations with a particular signature as well.
            // So here, we only deserialize the "promising" declarations (by FQ name), and record that someone
            // asked for a given signature, but postpone the actual linkage to post-processing.
            symbolsToLinkLater[idSig] = symbol
        }
    ...
    private fun findReferencedClassifier(classifier: KmClassifier, mayComeFromCinterop: Boolean, typeParametersInScope: Map<Int, IrTypeParameter> = emptyMap()): IrClassifierSymbol {
        return when (classifier) {
            ...
            is KmClassifier.Class -> findReferencedClass(classifier.name, mayComeFromCinterop)
    ...
    private fun findReferencedClass(className: ClassName, mayComeFromCinterop: Boolean): IrClassifierSymbol {
        ...
        if (mayComeFromCinterop) {
            ...
            linker.tryDeserializeSymbol(cinteropSignature, BinarySymbolData.SymbolKind.CLASS_SYMBOL, this@KonanInteropModuleDeserializerK2)
                    .first?.let { return it as IrClassifierSymbol }
        }
        ...
        return linker.deserializeOrReturnUnboundIrSymbolIfPartialLinkageEnabled(regularSignature, BinarySymbolData.SymbolKind.CLASS_SYMBOL,
                this@KonanInteropModuleDeserializerK2) as IrClassifierSymbol
    }
    ...
    private fun computeSignatureAndMaybeBind(declaration: IrDeclarationWithName) {
        ...
        val referencedSymbol = symbolsToLinkLater[signature]
        when (declaration) {
            is IrSimpleFunction -> {
                val newSymbol = referencedSymbol as IrSimpleFunctionSymbol? ?: IrSimpleFunctionSymbolImpl(signature = signature)
            ...
            is IrConstructor -> {
                val newSymbol = referencedSymbol as IrConstructorSymbol? ?: IrConstructorSymbolImpl(signature = signature)
                newSymbol.bind(declaration)
                (declaration as IrConstructorImpl).symbol = newSymbol
                symbolTable.declareConstructor(signature, { newSymbol }, { declaration })
            }
            is IrProperty -> {
                val newSymbol = referencedSymbol as IrPropertySymbol? ?: IrPropertySymbolImpl(signature = signature)
                newSymbol.bind(declaration)
    ...
    private fun deserializeAnnotation(kmAnnotation: KmAnnotation): IrAnnotation {
        val annotationClassSymbol = findReferencedClass(kmAnnotation.className, mayComeFromCinterop = false) as IrClassSymbol
        val annotationClass = linker.getDeclaration(annotationClassSymbol) as? IrClass
        val constructor = annotationClass?.constructors?.firstOrNull { ctor ->
            ctor.parameters.map { it.name.asString() }.toSet().containsAll(kmAnnotation.arguments.keys)
        }
        if (constructor == null) {
    ...
    private fun deserializeAnnotationArgument(kmArgument: KmAnnotationArgument, expectedType: IrType): IrExpression {
        return when (kmArgument) {
            ...
            is KmAnnotationArgument.KClassValue -> {
                val classSymbol = findReferencedClass(kmArgument.className, mayComeFromCinterop = true) as IrClassSymbol
                IrClassReferenceImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, builtIns.kClassClass.starProjectedType, classSymbol, classSymbol.defaultType)
            }
            is KmAnnotationArgument.EnumValue -> {
                ...
                val enumClassSymbol = findReferencedClass(kmArgument.enumClassName, mayComeFromCinterop = false) as IrClassSymbol
                val irType = enumClassSymbol.defaultType
            ...
            is KmAnnotationArgument.ArrayKClassValue -> TODO("Unexpected annotation argument kind usage inside C-interop Klib: class array")
        }
    }
    ...
    private fun KmType.toIrType(typeParametersInScope: Map<Int, IrTypeParameter> = emptyMap()): IrType {
        ...
        val classifier = findReferencedClassifier(classifier, mayComeFromCinterop = true, typeParametersInScope)
